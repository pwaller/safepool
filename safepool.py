...
def crash(i):
    "Segfault!"
    print "\x1B[31;1m" + "SEGFAULTING (job %r)!" % i + "\x1B[m"
    from ctypes import CDLL
    CDLL(None).strlen(None)

def crashing_function(i):
    "I can haz segfault?"
    
    print "  --> Inside job ", i
    from random import random
    if random() > 0.05 and 1:
        crash(i)
...
def main():
    p = SafePool(5); p.polltime = 0.1
    #p = Pool(5)
    
    try:
        #print "Result =", p.map(test, xrange(20))
        pass
    except:
    ...
    for i in count():
        ...
        print "Submitting job", i, len(gc.get_objects())
        
        #a = p.apply_async(crashing_function, [i])
        
        try:
            #result = a.get()
            result = p.apply(crashing_function, [i])
        except RuntimeError, e:
            print "Suffered a RuntimeError:", e.args
        else:
            print "Successfully got a result:", result
        print
